...
const Lc = () => {
  ...
  const fetchLikedCenters = async () => {
    try {
      ...
      if (!token) {
        setLikedCenters({});
        console.log("Token topilmadi, yoqtirilgan markazlar bo'sh.");
        return;
      }
      // Joriy foydalanuvchi ID'sini localStorage'dan olish
      const userData = JSON.parse(localStorage.getItem("user"));
      const currentUserId = userData?.id;
      console.log("Joriy foydalanuvchi ID:", currentUserId);

      const res = await axios.get("https://findcourse.net.uz/api/liked", {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log("API /api/liked javobi:", res.data);

      const likedMap = {};
      if (res.data && Array.isArray(res.data.data)) {
        // Faqat joriy foydalanuvchiga tegishli markazlarni filtrlash
        const userLikedItems = res.data.data.filter(item => {
          console.log(`Tekshirilmoqda: centerId=${item.centerId}, userId=${item.userId}`);
          return item.userId === currentUserId;
        });
        console.log("Filtrlangan yoqtirilgan markazlar (userLikedItems):");
        userLikedItems.forEach(item => {
          likedMap[item.centerId] = item.id; // likedItemId ni saqlash
          console.log(`  centerId: ${item.centerId}, likedItemId: ${item.id}`);
        });
      }
      setLikedCenters(likedMap);
      console.log("Yangilangan likedCenters state:", likedMap);
    } catch (err) {
  ...
  const handleLikeToggle = async (centerId) => {
    ...
    const likedItemId = likedCenters[centerId];
    try {
      if (likedItemId) {
        ...
        setLikedCenters(prev => {
          const newState = { ...prev };
          delete newState[centerId];
          return newState;
        });
      } else {
        const res = await axios.post("https://findcourse.net.uz/api/liked", { centerId }, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        {filteredCenters.map((center) => (
          ...
          >
            <div className="relative h-48">
              ...
              >
                <svg
                  className={`h-6 w-6 ${likedCenters[center.id] ? 'text-red-500' : 'text-gray-400'}`}
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
